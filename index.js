...
const summaryBlock =
  ...
  `🌿 算命学：${dayStem}（五行：${element}｜守護神：${guardianSpirit}）`;
    ...
    const prompt = `
以下の summaryBlock は、LINEの仕様により「1つのメッセージとして」送信する必要があります。
したがって、**このブロックは絶対に分割せず、1つのテキストメッセージとして1,800文字以内に収まるようにまとめて**出力してください。
途中で分割・改行・装飾を省略したりせず、**そのままの形式・装飾で1メッセージの中に保持**してください。
${summaryBlock}
